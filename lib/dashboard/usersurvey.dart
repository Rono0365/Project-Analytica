    ...
    return Scaffold(
      ...
      value: (item) => stacknotes().where((x)=>x["datetime"]==item["datetime"]).map((x)=>x["username"] ).toList().toSet().length>10?10:stacknotes().where((x)=>x["datetime"]==item["datetime"]).map((x)=>x["username"] ).toList().toSet().length,
              ...
              //  height: 350,
               ...
               // subtitle: Text(  mostRepeatedItems2( usersurvey.toString())),
                  //  subtitle:   Text(   usersurvey.length>0? mostRepeatedItems(usersurvey.toString()).toString():"usersurvey.toString()".toString()
                    ),
             //   Text('${countuniqueusers(stacknotes().where((x)=>x["datetime"]==leo).map((x)=>x["username"].toString()).toSet().toList(),"hitler","unique_user")}'),
            
                   // Text(  mostRepeatedItems(usersurvey).toString().replaceAll("[","").replaceAll("]","").toString()),
              ...
              //ge.toString())),
                  //  subtitle:   Text(   usersurvey.length>0? mostRepeatedItems(usersurvey.toString()).toString():"usersurvey.toString()".toString()
                //    ),
              //  r      Text(compareAndAddToList(usersurvey,['Opened app', 'tutorial','Exited app',  'Switched to dark mode', 'Switched to light mode', 'Clicked on', 'Changed theme to dark mode']).toString()),
