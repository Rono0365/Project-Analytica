...
class featureusage extends StatefulWidget {
  featureusage({
    super.key,
  ...
  final List pics;
  @override
  State<featureusage> createState() => _featureusageState();
}

class _featureusageState extends State<featureusage> {
  var information;
  var leo;
  var leo2;
  var matukios;
 ...
 Colors.grey.shade900.withOpacity(.6):Colors.grey.shade100.withOpacity(.8);
    return Scaffold(
              ...
              child: SingleChildScrollView(
                child: Column(
                 // alignment: WrapAlignment.spaceEvenly,
                  ...
                  children: [
                    ListTile(
                      title: Text("${leo}", style: GoogleFonts.rubik(
                            fontSize: 22,
      ...
      value: (item) => stacknotes().where((x)=>x["datetime"]==item["datetime"]).map((x)=>x["username"] ).toList().toSet().length>10?10:stacknotes().where((x)=>x["datetime"]==item["datetime"]).map((x)=>x["username"] ).toList().toSet().length,
                                                    ...
                                                    ListTile(title: Text("Analytics for ${stacknotes().where((x)=>x["datetime"]==leo).map((x)=>x["username"] ).toList().toSet().length} users",style: GoogleFonts.rubik(
                            fontSize: 22,
                            fontWeight: FontWeight.normal,
                            color: widget.darkmode ? Colors.white : Colors.black),),
              ...
              //  height: 350,
                                                  ...
                                                  child: Padding(
                                                    ...
                                                    child: SingleChildScrollView(
                                                      child: DataTable(
                                                                columns: [
                                                                  DataColumn(label: 
                                                       Text('Produce',style: GoogleFonts.paytoneOne(color: text1,fontWeight:FontWeight.normal))),
                                                                  DataColumn(label: Text('Count',style: GoogleFonts.paytoneOne(color: text1,fontWeight:FontWeight.normal))),
                                                                  DataColumn(label: Text('Users',style: GoogleFonts.paytoneOne(color: text1,fontWeight:FontWeight.normal))),
                                                               //   DataColumn(label: Text('Unique users',style: GoogleFonts.paytoneOne(color: text1,fontWeight:FontWeight.normal))),
                                                                  DataColumn(label: Text('FirstTime users',style: GoogleFonts.paytoneOne(color: text1,fontWeight:FontWeight.normal))),
                                                                  DataColumn(label: Text('Returning users',style: GoogleFonts.paytoneOne(color: text1,fontWeight:FontWeight.normal))), 
                                                      
                                                                ],
                                                                rows: [
                                                                 
                                                                ... ['carrot','spinach','samosa'].map((i)=>
                                                                  DataRow(cells: [//'Opened app', 'tutorial','Exited app',  'Switched to dark mode', 'Switched to light mode', 'Clicked on', 'Changed theme to dark mode'
                                                                    DataCell(Text('${i}',style: GoogleFonts.inconsolata(color: widget.darkmode? Colors.white:Colors.black,fontWeight:FontWeight.bold,fontSize:18))),//countAvailableItems(fruits, (fruit) => fruit.contains('a'));
                                                                    DataCell(Text(countAvailableItems(stacknotes().where((x)=>x["datetime"] == leo.toString() &&  x['username']!="").toList().map((i)=>
                  i["event"].replaceAll("{","").replaceAll("}","").replaceAll("[","").replaceAll("]","").split(",")
                    ).toList().map((i)=>i).toString().replaceAll("[", "").replaceAll("]", "").replaceAll("(", "").replaceAll(")", "").split(",").toList(), (fruit) => fruit.contains(i)).toString(),style: GoogleFonts.inconsolata(color: text1,fontWeight:FontWeight.normal))),
                                                                    DataCell(Text('${counrNusersofevent(stacknotes().where((x)=>x["datetime"]==leo).map((x)=>x["username"].toString()).toSet().toList(),"${i}").length}',style: GoogleFonts.inconsolata(color: text1,fontWeight:FontWeight.normal))),
                                                                  
                                                                  //  DataCell(Text('${countuniqueusers(stacknotes().where((x)=>x["datetime"]==leo).map((x)=>x["username"].toString()).toSet().toList(),"${i}","unique_user").length}',style: GoogleFonts.inconsolata(color: text1,fontWeight:FontWeight.normal))),
                                                                    DataCell(Text('${countuniqueusers(stacknotes().where((x)=>x["datetime"]==leo).map((x)=>x["username"].toString()).toSet().toList(),"${i}","firsttime_user").length}',style: GoogleFonts.inconsolata(color: text1,fontWeight:FontWeight.normal))),
                                                                    DataCell(Text('${countuniqueusers(stacknotes().where((x)=>x["datetime"]==leo).map((x)=>x["username"].toString()).toSet().toList(),"${i}","returning_user").length}',style: GoogleFonts.inconsolata(color: text1,fontWeight:FontWeight.normal))),
               ...
               // subtitle: Text(  mostRepeatedItems2( featureusage.toString())),
                  //  subtitle:   Text(   featureusage.length>0? mostRepeatedItems(featureusage.toString()).toString():"featureusage.toString()".toString()
                    ),
             //   Text('${countuniqueusers(stacknotes().where((x)=>x["datetime"]==leo).map((x)=>x["username"].toString()).toSet().toList(),"hitler","unique_user")}'),
            
                   // Text(  mostRepeatedItems(featureusage).toString().replaceAll("[","").replaceAll("]","").toString()),
              ...
              //ge.toString())),
                  //  subtitle:   Text(   featureusage.length>0? mostRepeatedItems(featureusage.toString()).toString():"featureusage.toString()".toString()
                //    ),
              //  r      Text(compareAndAddToList(featureusage,['Opened app', 'tutorial','Exited app',  'Switched to dark mode', 'Switched to light mode', 'Clicked on', 'Changed theme to dark mode']).toString()),
